 ...
 *
 * @details a program that prints out all the statistics of a very large given
 * list such as the min, max, size, average, and median. Using a struct this
 * time.
 *
 * @bugs none
 *
...
void print_summary(struct summary_t *prin);
void getsum(struct summary_t *conclud, size_t size, int a[]);
...
/**
 * Organizes various statistics by calling upon other fucntions.
 * @param conclud is the struct where data is assigned.
 * @param size the size of all of the data.
 * @param a[] all of the data being studied.
 */
void getsum(struct summary_t *conclud, size_t size, int a[])
{

        int leg[3] = {0,0,0};
        conclud->max = find_min(a, size);
        conclud->min = find_max(a, size);
        conclud->mid = midpoint(conclud->max,conclud->min);
        conclud->midlin = linear_search(a, conclud->mid, size);
        conclud->size = size;
        get_count(a,leg, conclud->mid, size);
        conclud->less = leg[0];
        conclud->larger = leg[2];
        conclud->odd = odd_count(a, size);
        conclud->even = even_count(a, size);
        conclud->divd = divisible_count(a, size, conclud->mid);
        conclud->medi = median(a, size);
        conclud->sums = sum(a, size);
        conclud->avg = average(a, size);
 ...
 * @param prin the struct that holds all of the stats
 */
void print_summary(struct summary_t *prin)
{
